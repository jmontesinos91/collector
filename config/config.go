...
type OmniViewConfigurations struct {
	Server                string `koanf:"server"`
	MaxRetries            int    `koanf:"max-retries"`
	TimeoutInSeconds      int64  `koanf:"timeout-in-seconds"`
	RetryWaitMinInSeconds int64  `koanf:"retry-wait-min-in-seconds"`
	RetryWaitMaxInSeconds int64  `koanf:"retry-wait-max-in-seconds"`
}
...
type Configurations struct {
	...
	OldDatabase DatabaseConfigurations             `koanf:"olddatabase"`
	OmniView    omnibackend.OmniViewConfigurations `koanf:"provider"`
...
func LoadConfig(logger *logger.ContextLogger) *Configurations {
	koa := koanf.New(".")
	err := koa.Load(file.Provider("resources/config.yml"), yaml.Parser())
	if err != nil {
	...
	err = koa.Load(env.Provider("", ".", func(s string) string {
	...
	err = koa.Unmarshal("", &configuration)
	if err != nil {
